...
type TestcasesProps = {
    problem: Problem;
    interviewerInputTestCase: string;
    interviewerOutputTestCase: string;
    fetchTestCaseData: () => void;
};

const Testcases: React.FC<TestcasesProps> = ({ problem, interviewerInputTestCase, interviewerOutputTestCase, fetchTestCaseData }) => {
            ...
            <div className='w-full  overflow-auto'>
                ...
                <div className='flex'>
                    {problem.examples.map((example, index) => (
                        <div className='mr-2 items-start mt-2 text-gray-500' key={example.id}
                            onClick={() => {
                                ...
                                setShowInterviewerTestCase(false);
                            }
                            }>
                     ...
                     ${activeTestCaseId === index && showInterviewerTestCase == false ? "text-white" : "text-gray-500"}`}>
                                    Case {index + 1}
                                </div>
                            </div>
                        </div>))}
                    ...
                    <div className='mr-2 items-start mt-2 text-gray-500' key='add-testcase'
                        onClick={() => {
                            setShowInterviewerTestCase(true);
                            fetchTestCaseData();
                        }
                        }
                    ...
                    you can't come up with a testcase yourself!">
                        <div className='flex flex-wrap items-center gap-y-4'>
                            <div className={`font-medium items-center transition-all focus:outline-none inline-flex bg-dark-fill-3
                        hover:bg-dark-fill-2 relative rounded-lg px-4 py-1 cursor-pointer whitespace-nowrap 
                        ${showInterviewerTestCase === true ? 'text-orange-400' : 'text-orange-200'}`}>
