...
const ResetPassword: React.FC<ResetPasswordProps> = () => {
	...
	const handleReset = async (e: React.FormEvent<HTMLFormElement>) => {
		e.preventDefault();
		const successful = await sendPasswordResetEmail(email);
		if (successful) {
			alert(`Password reset email sent to ${email}`); // will still send if email doesn't exist to prevent user enumeration
		}
	};
	useEffect(() => {
		if (error) {
